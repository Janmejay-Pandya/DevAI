...
def is_vertically_stacked(prev, curr):
    return (
        abs(prev['xmin'] - curr['xmin']) <= THRESHOLD and
        curr['ymin'] >= prev['ymax']
    )

def is_separate_column(prev, curr):
    return curr['xmin'] > prev['xmax'] + THRESHOLD

def build_layout(elements):
    elements.sort(key=lambda x: x['ymin'])
    row = { "type": "row", "children": [] }

    for element in elements:
        placed = False
        for column in row["children"]:
            last_elem = column["children"][-1]
            if is_vertically_stacked(last_elem, element):
                column["children"].append({ "type": element["name"] })
                placed = True
                break
